...
FLAVAOutput.__annotations__ = {
    ...
    "projected_image_embeddings": TransformerOutput,
    "projected_text_embeddings": TransformerOutput,
    "image_attentions": Optional[Tensor],
    "text_attentions": Optional[Tensor]}
...
class FLAVAModel(nn.Module):
    ...
    def forward(
        ...
        # image_encoding_out may be (TransformerOutput, projected_embeddings, image_attns)
        image_outputs = None
        projected_image_embeddings = None
        image_attentions = None
        if image_encoding_out is not None:
            if isinstance(image_encoding_out, tuple) and len(image_encoding_out) >= 2:
                # handle (TransformerOutput, projected_embeddings, maybe_attns)
                image_outputs = image_encoding_out[0]
                projected_image_embeddings = image_encoding_out[1]
                if len(image_encoding_out) >= 3:
                    image_attentions = image_encoding_out[2]
            else:
        ...
        # text_encoding_out may be (TransformerOutput, projected_embeddings, text_attns)
        text_outputs = None
        projected_text_embeddings = None
        text_attentions = None
        if text_encoding_out is not None:
            if isinstance(text_encoding_out, tuple) and len(text_encoding_out) >= 2:
                text_outputs = text_encoding_out[0]
                projected_text_embeddings = text_encoding_out[1]
                if len(text_encoding_out) >= 3:
                    text_attentions = text_encoding_out[2]
            else:
                text_outputs = text_encoding_out

        # ---- masked variants (no projection needed) ----
        ...
        if isinstance(image_masked_outputs, tuple):
            image_masked_outputs, image_masked_attns = image_masked_outputs
        else:
            image_masked_attns = None
    ...
    def encode_image(
        ...
        # try to request attention weights from the underlying image_encoder
        if image_patches_mask is not None:
            # pass the flag; underlying encoder may accept it
            encoded_image = self.image_encoder(image, image_patches_mask)
        else:
            encoded_image = self.image_encoder(image)

        # try to find attention tensor on the returned object
        image_attns = None
        if hasattr(encoded_image, "attn_weights"):
            image_attns = encoded_image.attn_weights
        elif hasattr(encoded_image, "attentions"):
            image_attns = encoded_image.attentions

        if projection:
            projected_embeddings = self.image_projection(
                encoded_image.last_hidden_state[:, 0, :]
            )
            # return TransformerOutput and projected embeddings; also attach attentions
            return (encoded_image, projected_embeddings, image_attns)
        # return TransformerOutput (with attn accessible) but also return attns explicitly
        return encoded_image, image_attns

    def encode_text(
        self, text: Tensor, text_mask: Optional[Tensor] = None, projection: bool = False
    ) -> Union[Tuple[TransformerOutput, Tensor], Optional[TransformerOutput]]:
        # text encoder already supports return_attn_weights=True in your original code.
        encoded_text = self.text_encoder(
            input_ids=text,
            attention_mask=text_mask
        )
        text_attns = None
        if hasattr(encoded_text, "attn_weights"):
            text_attns = encoded_text.attn_weights
        elif hasattr(encoded_text, "attentions"):
            text_attns = encoded_text.attentions
        ...
        if projection:
            ...
            return (encoded_text, projected_embeddings, text_attns)
        return encoded_text, text_attns
    ...
    def encode_mm(
        ...
        # request attention weights from mm encoder if supported
        # Many TransformerEncoder variants take return_attn_weights flag.
        mm_out = self.mm_encoder(fused_state)

        mm_attns = None
        if hasattr(mm_out, "attn_weights"):
            mm_attns = mm_out.attn_weights
        elif hasattr(mm_out, "attentions"):
            mm_attns = mm_out.attentions

        # Attach the mm_attns into the returned object by returning tuple
        return mm_out, mm_attns
